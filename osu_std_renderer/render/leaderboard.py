"""MAP RENDER LEADERBOARD — the per-map board on the results screen (owner
mockup 2026-07-09): the current play's full results card in the centre,
flanked by compact ranked cards of the OTHER renders of the SAME map, pulled
from the R3D render DB (LOCAL-ONLY — no osu!API, same rule as the PB card).

This module is the DATA + AVATAR layer (pure, unit-tested); the card BAKING
and per-frame DRAW live in render/lazer_results.py, reusing that screen's
Nunito loader, FOR_RANK grade colours and grade glow so the flanking cards
match the featured panel.
...
AVATAR_CACHE_DIR = os.path.expanduser("~/.cache/r3d-std/avatars")
...
def fetch_discord_avatar(discord_user_id: str, token: str,
    ...
    try:
        body = _https_get(
            ...
            {"Authorization": f"Bot {token}", "User-Agent": "r3d-std/1.0"},
            timeout, proxy_url)
        ...
        img = _https_get(
            ...
            {"User-Agent": "r3d-std/1.0"}, timeout, proxy_url)
        return img
