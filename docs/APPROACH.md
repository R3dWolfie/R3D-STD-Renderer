...
- **Precedent:** a reference renderer (a reference site) runs danser server-side at scale, publicly, with donation perks (4K unlock), no special arrangement documented or needed. — https://a reference site/donate · https://github.com/MasterIO02/ordr-client
...
- **Music is the elephant.** Nearly every MP4 we serve embeds commercially unlicensed songs; osu!'s own music licensing does not extend to third-party monetized redistribution. This risk is *identical* for danser, lazer, or our renderers. Mitigations: the DMCA machinery we already have (`renders.dmca_muted/dmca_notice` columns exist), takedown responsiveness, and a reference renderer-style framing of payments as supporter perks rather than "buying a video." — https://osu.ppy.sh/legal/en/Music_licensing · https://osu.ppy.sh/legal/en/Terms
...
- Per §1, buy a BASS license (likely the €125 shareware tier given supporter pricing), keep a reference renderer-style supporter framing, courtesy notes to Wieku/Haskorion → danser use becomes defensible indefinitely at near-zero cost.
...
5. **Legal residue independent of renderer:** music/art inside MP4s from a paid service. *Mitigate:* keep DMCA machinery sharp, a reference renderer-style supporter framing, never ship osu-resources assets, keep Argon *procedural*, trademark hygiene, send the two emails (§6). If danser remains in any paid path during transition: BASS license (€125 tier likely suffices).
